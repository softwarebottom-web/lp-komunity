...
auth.onAuthStateChanged(async (user) => {
    ...
    let role = "GUEST";
    if (user) {
        // Log Login ke Discord (Sekali per sesi)
    ...
    if (user) {
        ...
        if (role === "FOUNDER") menu += `<a href="ownerarea.html"><span>👑</span>OWNER</a>`;
...
window.loginDiscord = () => {
    ...
    auth.signInWithPopup(provider).then((res) => {
        ...
        }, { merge: true }).then(() => { window.location.href = "index.html"; });
